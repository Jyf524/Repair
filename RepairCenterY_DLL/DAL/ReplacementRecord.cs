...
namespace Maticsoft.DAL
{
	...
	public partial class ReplacementRecord
	{
		...
		public Maticsoft.Model.ReplacementRecord GetModel(string ReplacementRecordID)
		{
            ...
            strSql.Append(" where ReplacementID='" + ReplacementRecordID + "' ");
        ...
        public DataSet GetList1(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" * ");
            strSql.Append(" FROM ReplacementRecord a,UsersInfo b,UnitsInfo c ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere + " and a.UserID = b.UserID and a.GGG1 = c.UnitID " );
            }
            strSql.Append(" order by " + filedOrder);
            return DbHelperSQL.Query(strSql.ToString());
        }
