...
namespace Maticsoft.DAL
{
	...
	public partial class Declaration
	{
		...
		public bool Add(Maticsoft.Model.Declaration model)
		{
            ...
            if (model.MachineSonType != null)
			{
                strSql1.Append("MachineSonType,");
                strSql2.Append("'" + model.MachineSonType + "',");
			}
			if (model.MachineFatherType != null)
			{
				strSql1.Append("MachineFatherType,");
				strSql2.Append("'"+model.MachineFatherType+"',");
			}
			if (model.ReworkInstructions != null)
			{
				strSql1.Append("ReworkInstructions,");
				strSql2.Append("'"+model.ReworkInstructions+"',");
			}
			if (model.Acceptancetime != null)
			{
				strSql1.Append("Acceptancetime,");
				strSql2.Append("'"+model.Acceptancetime+"',");
			}
			if (model.CCC5 != null)
			{
				strSql1.Append("CCC5,");
		...
		public bool Update(Maticsoft.Model.Declaration model)
		{
			...
			if (model.MachineSonType != null)
			{
				strSql.Append("MachineSonType='"+model.MachineSonType+"',");
			}
			else
			{
				strSql.Append("MachineSonType= null ,");
			}
			if (model.MachineFatherType != null)
			{
				strSql.Append("MachineFatherType='"+model.MachineFatherType+"',");
			}
			else
			{
				strSql.Append("MachineFatherType= null ,");
			}
			if (model.ReworkInstructions != null)
			{
				strSql.Append("ReworkInstructions='"+model.ReworkInstructions+"',");
			}
			else
			{
				strSql.Append("ReworkInstructions= null ,");
			}
			if (model.Acceptancetime != null)
			{
				strSql.Append("Acceptancetime='"+model.Acceptancetime+"',");
			}
			else
			{
				strSql.Append("Acceptancetime= null ,");
			}
			if (model.CCC5 != null)
			{
		...
		public Maticsoft.Model.Declaration GetModel(string DeclarationID)
		{
			...
			strSql.Append(" DeclarationID,UserID,RepairerID,ListID,MachineName,DeclarationState,AssetsID,Model,OtherPart,BreakDown,ReplacementUse,ReplacementID,UnitName,RepairTime,Contact,ContactPhone,DoorServer,BeyondRepairDate,DeviceDescription,EstimateTime,FaultPrediction,RepairTreatment,RepairerName,TeacherName,TeacherID,RepairPlan,PartSource,PartPrice,ArrivalTime,Result,ResultTime,RepairPrice,PersonMoney,ActualMoney,MachineSonType,MachineFatherType,ReworkInstructions,Acceptancetime,CCC5 ");
		...
		public Maticsoft.Model.Declaration DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["MachineSonType"]!=null)
				{
					model.MachineSonType=row["MachineSonType"].ToString();
				}
				if(row["MachineFatherType"]!=null)
				{
					model.MachineFatherType=row["MachineFatherType"].ToString();
				}
				if(row["ReworkInstructions"]!=null)
				{
					model.ReworkInstructions=row["ReworkInstructions"].ToString();
				}
                if (row["Acceptancetime"] != null && row["Acceptancetime"].ToString() != "")
                {
                    model.Acceptancetime = DateTime.Parse(row["Acceptancetime"].ToString());
                }
				if(row["CCC5"]!=null)
				{
		...
		*/
        public DataSet GetList2(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *  ");
            strSql.Append("from  Declaration a,MachineSonType b,MachineFatherType c ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere + "and a.MachineSonType = b.MachineSonID and a.MachineFatherType = c.MachineFatherID order by RepairTime desc");
            }
            else
            {
                strSql.Append(" where a.MachineSonType = b.MachineSonID and a.MachineFatherType = c.MachineFatherID order by RepairTime desc");
            }
            return DbHelperSQL.Query(strSql.ToString());
        }
		#endregion  Method
        ...
        public DataSet GetListyyy (int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append("*");
            strSql.Append(" FROM   Declaration a,MachineSonType b,UnitsInfo c");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where a.MachineSonType = b.MachineSonID and a.UnitName=c.UnitName  and " + strWhere);
            }
            else
            {
                strSql.Append(" where Commodity.SubTypeID=Subcategory.SubTypeID and Subcategory.FType=ComType.FType");
            }
            strSql.Append(" order by " + filedOrder);
            return DbHelperSQL.Query(strSql.ToString());
        }
